...
//            int update = queryRunner.update(sql, "exam111000020180910", beanName + "_classgrade", index, "grade_id", s.getGrade_id());
            ...
            String sql = null;
            if (beanName.equals("totalscore")) {
                sql = "update " + tableName + " set  grade  = " + index + " where stu_id = " + s.getStu_id();

            } else {
                sql = "update " + tableName + " set " + beanName + "_grade" + " = " + index + " where stu_id = " + s.getStu_id();
            }
            System.out.println("年级排名开始：");
            System.out.println("=-=-=-=-=-" + sql);
            int update = queryRunner.update(sql);
            //System.out.println(s.getStu_name() + " 更新成功 : " + i);
            System.out.println("\t排号 :" + s.getGrade_id() + "\t学号 :" + s.getStu_id() + "\t科目 :" + beanName + "\t名次 :" + index + "\t分数 :" + getFieldValue(s, beanName) + "\t名字 :" + s.getStu_name());
...
//            int update = queryRunner.update(sql, "exam111000020180910", beanName + "_classgrade", index, "grade_id", s.getGrade_id());
            ...
            String sql = null;
            if (beanName.equals("totalscore")) {
                sql = "update " + tableName + " set  class_grade  = " + index + " where stu_id = " + s.getStu_id();

            } else {
                sql = "update " + tableName + " set " + beanName + "_classgrade" + " = " + index + " where stu_id = " + s.getStu_id();
            }
